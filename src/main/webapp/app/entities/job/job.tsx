...
export const Job = (props: IJobProps) => {
  ...
  return (
    <div>
      <h2 id="job-heading">
        <Translate contentKey="monolithicApp.job.home.title">Jobs</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="monolithicApp.job.home.createLabel">Create new Job</Translate>
        </Link>
      </h2>
        ...
        {jobList && jobList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('jobTitle')}>
                  <Translate contentKey="monolithicApp.job.jobTitle">Job Title</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('minSalary')}>
                  <Translate contentKey="monolithicApp.job.minSalary">Min Salary</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('maxSalary')}>
                  <Translate contentKey="monolithicApp.job.maxSalary">Max Salary</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="monolithicApp.job.employee">Employee</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {jobList.map((job, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{job.minSalary}</td>
                  <td>{job.maxSalary}</td>
                  <td>{job.employee ? <Link to={`employee/${job.employee.id}`}>{job.employee.firstName + ' ' + job.employee.lastName}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="monolithicApp.job.home.notFound">No Jobs found</Translate>
            </div>
          )
        )}
      </div>
